...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version 1.1.7}
\begin{itemize}
\item Parallelized partial Loess normalization with Permutation  added
\item several code improvements and safety checks
\item all Package building errors fixed
\item  Package cleaning
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version 1.1.2}
\begin{itemize}
\item Array Permutation over nodes added (intern function)
\item Parallelized Loess normalization added:
\Rfunction{normalizeAffyBatchLoessPara}
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version 0.99.2}
\begin{itemize}
\item Compilation error in vignette removed
\item Vignette extended
\item Object.type as input removed from all functions 
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version 0.99.0}
\begin{itemize}
\item Finalizing for BioConductor release.
\item Code cleaning.
\end{itemize}
...
\item Function \Rfunction{rmaPara} added.
\item Modularization.
\end{itemize}
...
\begin{itemize}
\item Testcode implemented and code testing -> several small bugs fixed
\end{itemize}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Version 0.1.0}
\begin{itemize}
\item Implementation / parallelization of funtions ...
	\begin{itemize}
	\item \Rfunction{bgCorrectPara}
	\item \Rfunction{normalizeAffyBatchConstantPara}, \Rfunction{normalizeAffyBatchInvariantsetPara}, \Rfunction{normalizeAffyBatchQuantilesPara}
	\item \Rfunction{computeExprSetPara}
	\item \Rfunction{preproPara}
	\item \Rfunction{distributeFiles}, \Rfunction{removeDistributedFiles}
	\end{itemize}
\end{itemize}
...
The function \Rfunction{bgCorrectPara} needs a cluster object (c1), an input data object (affybatch.example)
and the background correction method (method="rma") as input parameters.
\begin{Sinput}
R> affyBatchGBC <- bgCorrectPara(c1, affybatch.example,
					method="rma")
\end{Sinput}
...
The function \Rfunction{normalizeAffyBatchQuantilesPara} needs a cluster object (c1), an input data object (affybatch.example)
and quantil normalization parameters as input parameters (type = "pmonly").
\begin{Sinput}
R> affyBatchNORM <- normalizeAffyBatchQuantilesPara(c1,
		affybatch.example, type = "pmonly")
\end{Sinput}
...
The function \Rfunction{computeExprSetPara} needs a cluster object (c1), an input data object (affybatch.example)
...
The function \Rfunction{preproPara} needs a cluster object (c1), an input data object (affybatch.example)
...
\begin{Sinput}
R> esset <- rmaPara(c1,affybatch.example)
\end{Sinput}
...
\begin{Sinput}
R> affybatch1 <- bg.correct(affybatch.example,
		method="rma") 
R> affybatch2 <- bgCorrectPara(c1, affybatch.example,
		method="rma")
...
\begin{Sinput}
R> set.seed(1234)
R> affybatch1 <- normalize.AffyBatch.loess(c1, affybatch.example)
R> set.seed(1234)
R> affybatch2 <- normalizeAffyBatchLoessPara(affybatch.example) 
R> identical(exprs(affybatch1),exprs(affybatch2))
[1] TRUE
\end{Sinput}
