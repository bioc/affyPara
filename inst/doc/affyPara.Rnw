...
and versions \Sexpr{package.version("affy")} and \Sexpr{package.version("snow")} of the 
packages \Rpackage{affy} and \Rpackage{snow} respectively.
...
the main memory problems caused
by the \Robject{AffyBatch} object at only one workstation or processor. It is very difficult to define a concrete limit for
...
the literature or the webpage \url{http://www.cs.uiowa.edu/~luke/R/cluster/cluster.html}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Loading the package}
First of all you have to load the package.
<<echo=T, eval=T,results=hide>>=
library(affyPara)
@
For demonstration we use the small AffyBatch object \Robject{Dilution} from the \Rpackage{affydata} 
package:
<<echo=T, eval=T>>=
library(affydata)
data(Dilution)
...
@
with a parameter (4) for the number of spawend slaves and a parameter (type) for the used 
...
in the \Rpackage{affyPara} functions. But it is still possible!
...
mechanism (\Rpackage{Rmpi},\Rpackage{Rpvm}), and the implementation of your 
communication mechanism. 
...
The function \Rfunction{bgCorrectPara} needs a cluster object (cl), an input data object (Dilution)
...
to the slaves, see chapter \ref{chap:distri}. Additionally this example demonstrates how to use an 
extra cluster object (cl).
...
The function \Rfunction{normalizeAffyBatchQuantilesPara()} needs a cluster object (cl), an 
...
@
It is not very reasonable to have an \Robject{AffyBatch} as input data object for this function.
Because therefore you have to create a complete \Robject{AffyBatch} (very memory intensive).

It is much better to use a vector of CEL files as input data object. And at no point a complete \Robject{AffyBatch} needs
to be built:
...
Quality control and assessment get for a huge number of microarrays data very difficult. It takes 
